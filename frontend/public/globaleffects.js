...
/* ═══════════════════════════════════════════════════════════════════
   MOBILE NAV — injected globally by globalEffects.js
   Matches KIKK.be's approach: hamburger on mobile, horizontal on tablet+
   Reads the existing <nav> and <ul class="nav-links"> structure.
   No HTML changes needed across your pages.
═══════════════════════════════════════════════════════════════════ */
(function initMobileNav() {
  const nav = document.querySelector('nav');
  const navLinks = nav && nav.querySelector('.nav-links, ul');
  if (!nav || !navLinks) return;
  if (nav.dataset.mobileNav) return;
  nav.dataset.mobileNav = '1';

  // Build hamburger button
  const burger = document.createElement('button');
  burger.className = 'nav-hamburger mobile-only';
  ...
  if (!document.querySelector('link[href*="responsive.css"]')) {
    ...
    link.rel = 'stylesheet';
    // Resolve path relative to existing CSS links
    const existingLink = document.querySelector('link[href*="css/"]');
    const base = existingLink
      ? existingLink.getAttribute('href').replace(/css\/[^/]+$/, '')
      : 'css/';
    link.href = base + 'responsive.css';
    document.head.appendChild(link);
  }

  // Toggle open/close
  function toggleNav(open) {
    const isOpen = open !== undefined ? open : !navLinks.classList.contains('nav-open');
    navLinks.classList.toggle('nav-open', isOpen);
    burger.classList.toggle('nav-open', isOpen);
  ...
  // Close on nav link click (SPA-friendly)
  ...
  document.addEventListener('click', e => {
    if (!nav.contains(e.target)) toggleNav(false);
  });

  // Close on resize to tablet+
  const mq = window.matchMedia('(min-width: 768px)');
  mq.addEventListener('change', e => { if (e.matches) toggleNav(false); });
})();